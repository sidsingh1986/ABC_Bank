...
public interface CounterRepository extends JpaRepository<Counter, Integer> {
    ...
    @Query(value = "select coun.* from Counter coun join Counter_Services coun_ser on coun.id = coun_ser.Counter_id where coun.Branch_id = :branchId" +
            " AND coun.type_of_service = :customerType AND coun_ser.Services_id = :serviceId", nativeQuery = true)
    List<Counter> getCountersForService(Integer branchId, Integer serviceId, String customerType);
